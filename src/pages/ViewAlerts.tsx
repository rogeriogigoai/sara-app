...
const ViewAlerts = () => {
    const [alerts, setAlerts] = useState<DocumentData[]>([]);
    ...
    useEffect(() => {
        const fetchAlerts = async () => {
            try {
                ...
                const alertsData = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
        ...
        <div className="text-white">
            ...
            <div className="mt-8 bg-gray-800 rounded-lg shadow-lg">
                ...
                {!loading && alerts.length === 0 && (
                    <p className="p-6 text-center text-gray-500">Nenhum alerta encontrado. A frota está em conformidade!</p>
                )}
                                ...
                                <Link to={`/alert/${alert.id}`} className="flex items-center justify-between">
                                    <div className="flex items-center gap-4">
                                        <span className={`px-3 py-1 text-sm font-bold rounded-full ${
                                            alert.type === 'fraude' ? 'bg-red-500/20 text-red-400' : 'bg-yellow-500/20 text-yellow-400'
                                        }`}>
                                            {alert.type}
                                        </span>
                                        <div>
